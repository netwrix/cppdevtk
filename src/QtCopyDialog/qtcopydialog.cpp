...
void QtCopyDialogPrivate::error(int id, ::cppdevtk::qtsol::QtFileCopier::Error error, bool stopped)
{
...
void QtCopyDialogPrivate::stateChanged(::cppdevtk::qtsol::QtFileCopier::State state)
{
    Q_Q(QtCopyDialog);
...
void QtCopyDialog::setFileCopier(QtFileCopier *copier)
{
	...
	if (QMetaType::type("cppdevtk::qtsol::QtFileCopier::State")
#			if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
			== QMetaType::UnknownType) {
#			else
			== 0) {
#			endif
		if (qRegisterMetaType< ::cppdevtk::qtsol::QtFileCopier::State>("cppdevtk::qtsol::QtFileCopier::State")
#				if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
				== QMetaType::UnknownType) {
#				else
				== 0) {
#				endif
			throw CPPDEVTK_RUNTIME_EXCEPTION("failed to register metatype cppdevtk::qtsol::QtFileCopier::State");
		}
	}
	
    if (d->fileCopier) {
        disconnect(d->fileCopier, SIGNAL(error(int, ::cppdevtk::qtsol::QtFileCopier::Error, bool)),
                this, SLOT(error(int, ::cppdevtk::qtsol::QtFileCopier::Error, bool)));
        disconnect(d->fileCopier, SIGNAL(stateChanged(::cppdevtk::qtsol::QtFileCopier::State)),
                this, SLOT(stateChanged(::cppdevtk::qtsol::QtFileCopier::State)));
    ...
    if (d->fileCopier) {
        CPPDEVTK_VERIFY(connect(d->fileCopier, SIGNAL(error(int, ::cppdevtk::qtsol::QtFileCopier::Error, bool)),
                this, SLOT(error(int, ::cppdevtk::qtsol::QtFileCopier::Error, bool))));
        CPPDEVTK_VERIFY(connect(d->fileCopier, SIGNAL(stateChanged(::cppdevtk::qtsol::QtFileCopier::State)),
                this, SLOT(stateChanged(::cppdevtk::qtsol::QtFileCopier::State))));
