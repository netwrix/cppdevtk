      ...
      autoReset(true)
{
	...
	if (QMetaType::type("cppdevtk::qtsol::QtFileCopier::Error")
#			if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
			== QMetaType::UnknownType) {
#			else
			== 0) {
#			endif
		if (qRegisterMetaType< ::cppdevtk::qtsol::QtFileCopier::Error>("cppdevtk::qtsol::QtFileCopier::Error")
#				if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
				== QMetaType::UnknownType) {
#				else
				== 0) {
#				endif
			throw CPPDEVTK_RUNTIME_EXCEPTION("failed to register metatype cppdevtk::qtsol::QtFileCopier::Error");
		}
	}
	
    CPPDEVTK_VERIFY(connect(this, SIGNAL(error(int, ::cppdevtk::qtsol::QtFileCopier::Error, bool)),
                copier, SLOT(copyError(int, ::cppdevtk::qtsol::QtFileCopier::Error, bool))));
...
public:
    ...
    void copyError(int id, ::cppdevtk::qtsol::QtFileCopier::Error error, bool stopped);
...
void QtFileCopierPrivate::copyError(int id, ::cppdevtk::qtsol::QtFileCopier::Error error, bool stopped)
{
    Q_Q(QtFileCopier);
    if (stopped == true)
