...
#include <cppdevtk/jni/get_package_name.hpp>
...
#	endif
	...
	QAndroidJniObject packageName = applicationContext.callObjectMethod("getPackageName", "()Ljava/lang/String;");
	ThrowCppExceptionFromJavaPendingException(pJniEnv);
	if (!packageName.isValid() || packageName.toString().isEmpty()) {
		throw CPPDEVTK_JNI_EXCEPTION("failed to get jni packageName");
	}
	//CPPDEVTK_LOG_DEBUG("packageName: " << packageName.toString());
