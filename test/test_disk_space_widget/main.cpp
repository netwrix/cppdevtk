...
#	endif
	...
	::cppdevtk::test_disk_space_widget::InitResources();
	...
	Application::SetInfo(CPPDEVTK_COMPANY_SHORT_NAME_SANITIZED, CPPDEVTK_COMPANY_HOMEPAGE,
			CPPDEVTK_SHORT_NAME_SANITIZED, CPPDEVTK_VERSION_STRING, CPPDEVTK_TEST_DISK_SPACE_WIDGET_SHORT_NAME_SANITIZED);
	
	Application application(argc, argv);
	
	Application::setWindowIcon(QIcon(":/cppdevtk/test_disk_space_widget/res/ico/application.ico"));
	
#	if (CPPDEVTK_ENABLE_LOG_TO_FILE)
	if (kIsLogFileMsgHandlerInstalled) {
		MessageBox::Information(application.GetDefaultWindow(), CPPDEVTK_TEST_DISK_SPACE_WIDGET_SHORT_NAME,
				QString("Log file: %1").arg(QDir::toNativeSeparators(::cppdevtk::base::GetLogFileName())));
	}
	else {
		MessageBox::Critical(application.GetDefaultWindow(), CPPDEVTK_TEST_DISK_SPACE_WIDGET_SHORT_NAME,
				"InstallLogFileMsgHandler() failed");
	}
#	endif
	
	try {
		::cppdevtk::test_disk_space_widget::Widget widget;
		widget.show();
		...
		MessageBox::Critical(application.GetDefaultWindow(),
				CPPDEVTK_TEST_DISK_SPACE_WIDGET_SHORT_NAME,
