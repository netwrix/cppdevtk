...
include(./test_invisible_widget_customization.pri)
SAVED_TARGET = $${TARGET}
include(./../../common.pri)
...
RESOURCES += cppdevtk_test_invisible_widget.qrc	\
	cppdevtk_test_invisible_widget_customization.qrc

win32 {
	RC_FILE = cppdevtk_test_invisible_widget.rc
	HEADERS += cppdevtk_test_invisible_widget_res_win.hpp
}
		...
		CONFIG(debug, debug|release) {
			macx {
				cppdevtk_enable_app_target_debug_suffix {
					QMAKE_INFO_PLIST = ./cppdevtk_test_invisible_widget_mac_debug.plist
				}
				else {
					QMAKE_INFO_PLIST = ./cppdevtk_test_invisible_widget_mac.plist
				}
			}
			else {
				cppdevtk_enable_app_target_debug_suffix {
					QMAKE_INFO_PLIST = ./cppdevtk_test_invisible_widget_ios_debug.plist
				}
				else {
					QMAKE_INFO_PLIST = ./cppdevtk_test_invisible_widget_ios.plist
				}
			}
		}
		else {
