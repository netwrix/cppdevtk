...
include(./test_slot_throw_customization.pri)
SAVED_TARGET = $${TARGET}
include(./../../common.pri)
...
RESOURCES += cppdevtk_test_slot_throw.qrc	\
	cppdevtk_test_slot_throw_customization.qrc

win32 {
	RC_FILE = cppdevtk_test_slot_throw.rc
	HEADERS += cppdevtk_test_slot_throw_res_win.hpp
}
		...
		CONFIG(debug, debug|release) {
			macx {
				cppdevtk_enable_app_target_debug_suffix {
					QMAKE_INFO_PLIST = ./cppdevtk_test_slot_throw_mac_debug.plist
				}
				else {
					QMAKE_INFO_PLIST = ./cppdevtk_test_slot_throw_mac.plist
				}
			}
			else {
				cppdevtk_enable_app_target_debug_suffix {
					QMAKE_INFO_PLIST = ./cppdevtk_test_slot_throw_ios_debug.plist
				}
				else {
					QMAKE_INFO_PLIST = ./cppdevtk_test_slot_throw_ios.plist
				}
			}
		}
		else {
