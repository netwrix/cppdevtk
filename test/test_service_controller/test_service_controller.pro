...
include(./test_service_controller_customization.pri)
SAVED_TARGET = $${TARGET}
include(./../../common.pri)


!isEqual(CPPDEVTK_ENABLE_QTSOLUTIONS, "true") {
	error("This subproject require Qt Solutions enabled!!!")
}
...
cppdevtk_enable_target_suffix_qt_major_version {
	CPPDEVTK_GUI_TARGET = $${CPPDEVTK_GUI_TARGET}_qt$${QT_MAJOR_VERSION}
...
RESOURCES += cppdevtk_test_service_controller.qrc	\
	cppdevtk_test_service_controller_customization.qrc

win32 {
	RC_FILE = cppdevtk_test_service_controller.rc
	HEADERS += cppdevtk_test_service_controller_res_win.hpp
}
		...
		CONFIG(debug, debug|release) {
			macx {
				cppdevtk_enable_app_target_debug_suffix {
					QMAKE_INFO_PLIST = ./cppdevtk_test_service_controller_mac_debug.plist
				}
				else {
					QMAKE_INFO_PLIST = ./cppdevtk_test_service_controller_mac.plist
				}
			}
			else {
				cppdevtk_enable_app_target_debug_suffix {
					QMAKE_INFO_PLIST = ./cppdevtk_test_service_controller_ios_debug.plist
				}
				else {
					QMAKE_INFO_PLIST = ./cppdevtk_test_service_controller_ios.plist
				}
			}
		}
		else {
